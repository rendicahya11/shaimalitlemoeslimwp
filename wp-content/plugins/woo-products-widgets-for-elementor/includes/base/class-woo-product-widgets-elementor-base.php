...
abstract class WPFE_Woo_Widgets_Base extends Widget_Base {
	...
	public function __get_edit_looped_template( $templates = array(), $setting = null ) {
		?>
		<# if ( settings.<?php echo sanitize_key($setting); ?> ) { #>
		<?php
			if ( ! empty( $templates['start'] ) ) {
				include $templates['start'];
			}
		?>
			<# _.each( settings.<?php echo sanitize_key($setting); ?>, function( item ) { #>
			<?php
	...
	public function __edit_loop_item( $keys = array(), $format = '%s' ) {
		...
		echo '<# if ( item.' . sanitize_key($settings) . ' ) { #>';
		printf( $format, '{{{ item.' . sanitize_key($settings) . ' }}}' );
		echo '<# } #>';
	...
	public function __edit_inc_if( $file = null, $settings = array() ) {
		...
		<# if ( <?php echo sanitize_key($condition); ?> ) { #>
